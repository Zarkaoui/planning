...
if (isset($_REQUEST["champ_util"])) {
	...
	if ($db->numRows($requeteUtil) != 0) {	
		...
		for ($n = 0 ; $n < count($requeteUtil); $n++){
			echo "
			<tr id=\"miniFicheUtil\" onclick=\"document.location.replace('index.php?action=affCal&amp;year=$year&amp;month=$month&amp;sectID=$sectID&amp;entID=$entID&amp;utilid=".$requeteUtil[$n]["emp_id"]."');\" >
